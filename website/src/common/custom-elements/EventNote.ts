...
export namespace EventNote {
    ...
    export async function applyComponents(note: EventNote, ...components: EventInfo.Component[]) {
        ...
        for (const comp of components) {
            ...
            else if (comp instanceof EventInfo.Components.DeregistrationEnd) {
                await Promise.all([
                    onAuth(),
                    EventInfo.Components.Registerable.checkCanDeregister()
                ])
                    .then(([user, hasDeregisterPermission]) => {
                        if (user) { // is logged in
                            const isRegistered = note.event.getComponent(EventInfo.Components.Registerable)!.isRegistered(user.uid);

                            if (isRegistered && hasDeregisterPermission && comp.isNow()) {
                                const regButton = note.getElementsByClassName("registration-button")[0] as HTMLButtonElement | null;

                                if (regButton) {
                                    regButton.firstElementChild!.textContent = "lock_clock";
                                    regButton.lastElementChild!.textContent = "Uitschrijven is niet meer mogelijk";
    ...
    export namespace applyComponents {
        ...
        export const ORDER: Class<EventInfo.Component>[] = [
            ...
            EventInfo.Components.DeregistrationEnd,
